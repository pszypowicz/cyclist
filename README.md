...
- **Window and focus detection** follows [AltTab](https://github.com/lwouis/alt-tab-macos) and [yabai](https://github.com/koekeishiya/yabai): reading focus changes, Space membership, and real-window state from the WindowServer's own notification stream and window records instead of the Accessibility API, the tag/attribute predicate that filters out a fullscreen Space's companion windows, and AltTab's precedent for gating Quit on Finder.
