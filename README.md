...
<p align="center">
  <picture>
    ...
    <img src="docs/demo-light.svg" width="830" alt="Hold Cmd to open a vertical text list where every window is its own row, some tagged 'other space'; move the selection to a Safari window that lives in a fullscreen Space, release Cmd, and the display jumps straight to that Space.">
  </picture>
</p>
...
- A press that finds nothing to cycle still opens the panel, with one line saying so (`No windows found for Safari`). It closes when you release the binding, or on its own shortly after when the binding was already up. Without it, a press that found nothing looks exactly like a hotkey that never registered.
